		...
		switch ($act) {
			...
			case 'addsp':
				...
				if (isset($_POST['themmoi'])&&($_POST['themmoi'])) {
					...
					if ($pricePro < $delPro ) {
						$sql = 'INSERT INTO product(name,price,price_del,thumbnail,content,bestseller,sale,new,id_users,id_brand,created_at,updated_at) Values("'.$namePro.'","'.$pricePro.'","'.$delPro.'","'.$image.'","'.$descriptionPro.'","'.$bestseller.'","'.$sale.'","'.$new.'","'.$id_users.'","'.$id_brand.'","'.$created_at.'","'.$updated_at.'")';
					...
					{
						$thongbao['delPro'] = 'Không hợp lệ, Giá gốc phải lớn hơn giá sản phẩm.';
					}
				}
			...
			case 'updatesp':
				...
				if (isset($_POST['capnhat'])&&($_POST['capnhat'])) {
					...
					if (move_uploaded_file($_FILES['image']['tmp_name'], $target_file))
					{
	  				...
	  				{
	  					$sql = 'UPDATE product SET name = "'.$name.'",price ="'.$price.'",price_del ="'.$price_del.'",thumbnail ="'.$imageText.'", updated_at ="'.$updated_at.'"WHERE id = '.$id;
						pdo_execute($sql);
	  				}
